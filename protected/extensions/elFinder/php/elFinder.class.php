...
class elFinder {
	...
	protected function _mkdir()
	{
        ...
        # AlexLcDee patch to fix cyrillic names
        //$_GET['name']=$this->_translitName($_GET['name']);
    ...
    /**
     * Performs transliteration of file/dir name.
     * (need to store only latin symbols)
     * @param string $name
     * @return string
     */
    private function _translitName($name)
    {
        $matrix = array(
        ...
        foreach($matrix as $from => $to)
            $name = mb_eregi_replace($from, $to, $name);
